...
fun minimumOf(number1: Long, number2: Long){
    ...
    if(number1 < number2) {
        ...
        println("$number1 is greater or equals to $number2")
    }
}
...
fun rangeOf(number: Int){
    ...
    when(number) {
        in Int.MIN_VALUE..0 -> println("$number is 0 or below")
        ...
        else -> println("$number is 21 or above")
    }
}
...
fun inferredTypeOf(variable: Any){
    ...
    if(variable is Int || variable is Long || variable is Double ||
            variable is Float || variable is Boolean) {
        println("Data type of variable $variable is ${variable.javaClass}")
    } else {
        println("Data type of variable $variable is something else")
    }
}
